...
date_default_timezone_set('Asia/Colombo'); 
			...
			$app_id				=	trim($rowe['app_id']);

			$app_pswd			=	trim($rowe['app_password']);
...
define('SUBSCRIPTION_BASE_URL', 'https://api.mspace.lk/subscription/query-base/'); 

define('SUBSCRIPTION_BASE_URL', 'https://api.mspace.lk/subscription/query-base/');

define('SMS_SENDER_URL', 'https://api.mspace.lk/sms/send');

define('USSD_SENDER_URL', 'https://api.mspace.lk/ussd/send/');

define('SUBSCRIPTION_URL', 'https://api.mspace.lk/subscription/send/');

define('SUBSCRIPTION__BASE_URL', 'https://api.mspace.lk/subscription/query-base/');
...
function  createUser($address,$statustext,$statusnum,$agent){ 	
	...
	//$msg1 ="Await for the new updates or Dial ".APP_USSD." to view availbale contents."; 
...
$sql_job		=	"SELECT * FROM ".$key."_job_list WHERE cat_id='$cat_id' ORDER BY dtime DESC LIMIT 1 "; 
...
$row_job 	= 	mysqli_fetch_array($reply_job); 
...
$jobCat = "No Jobs under this category."; 
